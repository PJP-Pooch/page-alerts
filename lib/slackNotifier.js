 ...
 */
async function sendDailySummaryNotification(webhookUrl, results, settings = {}, project = null) {
  if (!webhookUrl) {
  ...
  // Header & Title based on project type
  let title = '';
  if (project) {
    const isOwn = project.type === 'own_site';
    const pIcon = isOwn ? '🏠' : '🎯';
    const siteLabel = isOwn ? 'Own Site' : 'Competitor';
    title = hasAnyChanges
      ? `${pIcon} ${project.name}: Changes Detected`
      : baselineCompetitors.length > 0
        ? `📦 ${project.name}: Sitemap Baseline Created`
        : `✅ ${project.name}: No Changes Detected`;
  } else {
    title = hasAnyChanges
  ...
  const sitesLabel = project?.type === 'own_site' ? 'Monitored Sites' : 'Competitors';
  blocks.push({
    type: 'section',
    text: {
      type: 'mrkdwn',
      text: `${projectLabel}*${sitesLabel}:* ${results.length} | *🟢 New Pages:* +${totalAddedAll} | *🔴 Removed Pages:* -${totalRemovedAll}\n_Check completed at ${new Date().toLocaleString()}_`
    }
  });
